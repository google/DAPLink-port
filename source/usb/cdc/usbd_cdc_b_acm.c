...
/** \ingroup  USBD_CDC_B_ACM_instance_variables
    \defgroup USBD_CDC_B_ACM_INSTANCE_VAR  Instance Variables (INSTANCE_VAR)
    \brief      Instance variables used in USBD CDC ACM module
 */
static int32_t data_send_access;              /*!< Flag active while send data (in the send intermediate buffer) is being accessed */
static int32_t data_send_active;              /*!< Flag active while data is being sent */
static int32_t data_send_zlp;                 /*!< Flag active when ZLP needs to be sent */
static int32_t data_to_send_wr;               /*!< Number of bytes written to the send intermediate buffer */
static int32_t data_to_send_rd;               /*!< Number of bytes read from the send intermediate buffer */
static uint8_t *ptr_data_to_send;             /*!< Pointer to the send intermediate buffer to the data to be sent */
static uint8_t *ptr_data_sent;                /*!< Pointer to the send intermediate buffer to the data already sent */

static int32_t data_read_access;              /*!< Flag active while read data (in the receive intermediate buffer) is being accessed */
static int32_t data_receive_int_access;       /*!< Flag active while read data (in the receive intermediate buffer) is being accessed from the IRQ function*/
static int32_t data_received_pending_pckts;   /*!< Number of packets received but not handled (pending) */
static int32_t data_no_space_for_receive;     /*!< Flag active while there is no more space for reception */
static uint8_t *ptr_data_received;            /*!< Pointer to the receive intermediate buffer to the received unread data */
static uint8_t *ptr_data_read;                /*!< Pointer to the receive intermediate buffer to the received read data */

static uint16_t control_line_state;           /*!< Control line state settings bitmap (0. bit - DTR state, 1. bit - RTS state) */

static CDC_LINE_CODING s_line_coding;           /*!< Communication settings */

/* end of group USBD_CDC_B_ACM_INSTANCE_VAR */


/* Functions that should be provided by user to use standard Virtual COM port
   functionality                                                              */
__WEAK int32_t USBD_CDC_B_ACM_PortInitialize(void)
{
    main_cdc_b_send_event();
    return 1;
}
__WEAK int32_t USBD_CDC_B_ACM_PortUninitialize(void)
{
    return 1;
}
__WEAK int32_t USBD_CDC_B_ACM_PortReset(void)
{
    return 1;
}
__WEAK int32_t USBD_CDC_B_ACM_PortSetLineCoding(CDC_LINE_CODING *line_coding)
{
    s_line_coding = *line_coding;
    return 1;
}
__WEAK int32_t USBD_CDC_B_ACM_PortGetLineCoding(CDC_LINE_CODING *line_coding)
{
    *line_coding = s_line_coding;
    return 1;
}
__WEAK int32_t USBD_CDC_B_ACM_PortSetControlLineState(uint16_t ctrl_bmp)
{
    return 1;
}
...
/* Functions that can be used by user to use standard Virtual COM port
   functionality                                                              */
int32_t USBD_CDC_B_ACM_DataSend(const uint8_t *buf, int32_t len);
int32_t USBD_CDC_B_ACM_PutChar(const uint8_t  ch);
int32_t USBD_CDC_B_ACM_DataRead(uint8_t *buf, int32_t len);
int32_t USBD_CDC_B_ACM_GetChar(void);
__WEAK int32_t USBD_CDC_B_ACM_DataReceived(int32_t len)
{
    return (0);
}
int32_t USBD_CDC_B_ACM_DataAvailable(void);
int32_t USBD_CDC_B_ACM_Notify(uint16_t stat);

/* Functions handling CDC ACM requests (can be overridden to provide custom
   handling of CDC ACM requests)                                              */
__WEAK int32_t USBD_CDC_B_ACM_SendEncapsulatedCommand(void)
{
    return (0);
}
__WEAK int32_t USBD_CDC_B_ACM_GetEncapsulatedResponse(void)
{
    return (0);
}
__WEAK int32_t USBD_CDC_B_ACM_SetCommFeature(uint16_t feat)
{
    return (0);
}
__WEAK int32_t USBD_CDC_B_ACM_GetCommFeature(uint16_t feat)
{
    return (0);
}
__WEAK int32_t USBD_CDC_B_ACM_ClearCommFeature(uint16_t feat)
{
    return (0);
}
__WEAK int32_t USBD_CDC_B_ACM_SendBreak(uint16_t dur)
{
    return 1;
}
...
static void USBD_CDC_B_ACM_EP_BULKOUT_HandleData(void);
static void USBD_CDC_B_ACM_EP_BULKIN_HandleData(void);
...
/** \brief  Initialization of the USB CDC class (ACM)
    ...
    The function calls USBD_CDC_B_ACM_PortInitialize function which
...
__WEAK int32_t USBD_CDC_B_ACM_Initialize(void)
{
    data_send_access            = 0;
    data_send_active            = 0;
    data_send_zlp               = 0;
    data_to_send_wr             = 0;
    data_to_send_rd             = 0;
    ptr_data_to_send            = USBD_CDC_B_ACM_SendBuf;
    ptr_data_sent               = USBD_CDC_B_ACM_SendBuf;
    data_read_access            = 0;
    data_receive_int_access     = 0;
    data_received_pending_pckts = 0;
    data_no_space_for_receive   = 0;
    ptr_data_received           = USBD_CDC_B_ACM_ReceiveBuf;
    ptr_data_read               = USBD_CDC_B_ACM_ReceiveBuf;
    control_line_state          = 0;
    s_line_coding.dwDTERate       = 9600;
    s_line_coding.bCharFormat     = 0;
    s_line_coding.bParityType     = 0;
    s_line_coding.bDataBits       = 8;
    return (USBD_CDC_B_ACM_PortInitialize());
}


/** \brief  Uninitialization of the USB CDC class (ACM)

    The function calls USBD_CDC_B_ACM_PortUninitialize function which
...
__WEAK int32_t USBD_CDC_B_ACM_Uninitialization(void)
{
    return (USBD_CDC_B_ACM_PortUninitialize());
}
...
/** \brief  Reset of the USB CDC class (ACM) variables and states
    ...
    USBD_CDC_B_ACM_PortReset function which resets Virtual COM Port variables
    and states and calls USBD_CDC_B_ACM_PortSetLineCoding function with
...
__WEAK int32_t USBD_CDC_B_ACM_Reset(void)
{
    data_send_access            = 0;
    data_send_active            = 0;
    data_send_zlp               = 0;
    ...
    ptr_data_read               = USBD_CDC_B_ACM_ReceiveBuf;
    control_line_state          = 0;
    USBD_CDC_B_ACM_PortReset();
    s_line_coding.dwDTERate       = 9600;
    s_line_coding.bCharFormat     = 0;
    s_line_coding.bParityType     = 0;
    s_line_coding.bDataBits       = 8;
    return (USBD_CDC_B_ACM_PortSetLineCoding(&s_line_coding));
}
...
__WEAK int32_t USBD_CDC_B_ACM_SetLineCoding(void)
{
    s_line_coding.dwDTERate   = (USBD_EP0Buf[0] <<  0) |
                              (USBD_EP0Buf[1] <<  8) |
                              (USBD_EP0Buf[2] << 16) |
                              (USBD_EP0Buf[3] << 24) ;
    s_line_coding.bCharFormat =  USBD_EP0Buf[4];
    s_line_coding.bParityType =  USBD_EP0Buf[5];
    s_line_coding.bDataBits   =  USBD_EP0Buf[6];
    return (USBD_CDC_B_ACM_PortSetLineCoding(&s_line_coding));
}
...
__WEAK int32_t USBD_CDC_B_ACM_GetLineCoding(void)
{
    if (USBD_CDC_B_ACM_PortGetLineCoding(&s_line_coding)) {
        USBD_EP0Buf[0] = (s_line_coding.dwDTERate >>  0) & 0xFF;
        USBD_EP0Buf[1] = (s_line_coding.dwDTERate >>  8) & 0xFF;
        USBD_EP0Buf[2] = (s_line_coding.dwDTERate >> 16) & 0xFF;
        USBD_EP0Buf[3] = (s_line_coding.dwDTERate >> 24) & 0xFF;
        USBD_EP0Buf[4] =  s_line_coding.bCharFormat;
        USBD_EP0Buf[5] =  s_line_coding.bParityType;
        USBD_EP0Buf[6] =  s_line_coding.bDataBits;
        return (1);
    }
...
__WEAK int32_t USBD_CDC_B_ACM_SetControlLineState(uint16_t ctrl_bmp)
{
    control_line_state = ctrl_bmp;
    return (USBD_CDC_B_ACM_PortSetControlLineState(ctrl_bmp));
}
...
*/
int32_t USBD_CDC_B_ACM_DataFree(void)
{
    return ((int32_t)usbd_cdc_b_acm_sendbuf_sz) - (data_to_send_wr - data_to_send_rd);
}
...
int32_t USBD_CDC_B_ACM_DataSend(const uint8_t *buf, int32_t len)
{
    ...
    len_available = ((int32_t)usbd_cdc_b_acm_sendbuf_sz) - len_data;  /* Num of
    ...
    if ((ptr_data_to_send >= ptr_data_sent) && /* If wrap is possible to happen */
            ((ptr_data_to_send + len) >= (USBD_CDC_B_ACM_SendBuf + usbd_cdc_b_acm_sendbuf_sz))) {
        /* If data wraps around end of buffer */
        len_before_wrap   = USBD_CDC_B_ACM_SendBuf + usbd_cdc_b_acm_sendbuf_sz - ptr_data_to_send;
        ...
        ptr_data_to_send  = USBD_CDC_B_ACM_SendBuf;       /* Wrap send buffer
...
int32_t USBD_CDC_B_ACM_PutChar(const uint8_t ch)
{
    if ((USBD_CDC_B_ACM_DataSend(&ch, 1)) == 1) {
        return ((uint32_t) ch);
    }
...
int32_t USBD_CDC_B_ACM_GetChar(void)
{
    ...
    if ((USBD_CDC_B_ACM_DataRead(&ch, 1)) == 1) {
        return ((int32_t) ch);
    }
...
int32_t USBD_CDC_B_ACM_Notify(uint16_t stat)
{
    if (USBD_Configuration) {
        USBD_CDC_B_ACM_NotifyBuf[0] = 0xA1;   /* bmRequestType                      */
        USBD_CDC_B_ACM_NotifyBuf[1] = CDC_NOTIFICATION_SERIAL_STATE;/* bNotification
                                          (SERIAL_STATE)                      */
        USBD_CDC_B_ACM_NotifyBuf[2] = 0x00;   /* wValue                             */
        USBD_CDC_B_ACM_NotifyBuf[3] = 0x00;
        USBD_CDC_B_ACM_NotifyBuf[4] = 0x00;   /* wIndex (Interface 0)               */
        USBD_CDC_B_ACM_NotifyBuf[5] = 0x00;
        USBD_CDC_B_ACM_NotifyBuf[6] = 0x02;   /* wLength                            */
        USBD_CDC_B_ACM_NotifyBuf[7] = 0x00;
        USBD_CDC_B_ACM_NotifyBuf[8] = stat >> 0; /* UART State Bitmap                  */
        USBD_CDC_B_ACM_NotifyBuf[9] = stat >> 8;
        /* Write notification to be sent      */
        USBD_WriteEP(usbd_cdc_b_acm_ep_intin | 0x80, USBD_CDC_B_ACM_NotifyBuf, 10);
        return (1);
    }
...
void USBD_CDC_B_ACM_Reset_Event(void)
{
    USBD_CDC_B_ACM_Reset();
}
...
/** \brief  Handle SOF Events
    ...
    (USBD_CDC_B_ACM_EP_BULKOUT_HandleData) if there is enough space in the
    intermediate receive buffer and it calls received function callback
    (USBD_CDC_B_ACM_DataReceived) it also activates data send over the Bulk In
    endpoint if there is data to be sent (USBD_CDC_B_ACM_EP_BULKIN_HandleData).
 */

void USBD_CDC_B_ACM_SOF_Event(void)
{
    if (!USBD_Configuration) {
    ...
    if ((!data_read_access)         &&    /* If not read active                 */
        ...
        ptr_data_received = USBD_CDC_B_ACM_ReceiveBuf;  /* Correct received pointer
                                                     to point to the start of
                                                     the receive buffer       */
        ptr_data_read     = USBD_CDC_B_ACM_ReceiveBuf;  /* Correct read pointer to
    ...
    if (data_received_pending_pckts &&    /* If packets are pending             */
        ...
        USBD_CDC_B_ACM_EP_BULKOUT_HandleData(); /* Handle received data             */
        data_read_access = 0;               /* Enable access to read data         */

        if (ptr_data_received != ptr_data_read) {
            USBD_CDC_B_ACM_DataReceived(ptr_data_received - ptr_data_read);
        }  /* Call
...
//&& ((control_line_state & 3) == 3)    /* and if DTR and RTS is 1            */
        ...
        USBD_CDC_B_ACM_EP_BULKIN_HandleData();/* Handle data to send                */
...
void USBD_CDC_B_ACM_EP_INTIN_Event(uint32_t event)
{
...
static void USBD_CDC_B_ACM_EP_BULKOUT_HandleData()
{
    uint32_t len_free_to_recv;
    int32_t len_received;

    if ((usbd_cdc_b_acm_receivebuf_sz - (ptr_data_received - USBD_CDC_B_ACM_ReceiveBuf)) >= usbd_cdc_b_acm_maxpacketsize1[USBD_HighSpeed]) {
        /* If there is space for 1 max packet */
        /* Read received packet to receive buf*/
        len_free_to_recv = usbd_cdc_b_acm_receivebuf_sz - (ptr_data_received - USBD_CDC_B_ACM_ReceiveBuf);
        len_received       = USBD_ReadEP(usbd_cdc_b_acm_ep_bulkout, ptr_data_received, len_free_to_recv);
...
static void USBD_CDC_B_ACM_EP_BULKIN_HandleData(void)
{
    ...
    if (len_to_send) {
        ...
        if ((ptr_data_sent >= ptr_data_to_send) && /* If data before end of buf avail*/
                ((ptr_data_sent + len_to_send) >= (USBD_CDC_B_ACM_SendBuf + usbd_cdc_b_acm_sendbuf_sz))) {
            ...
            len_to_send = USBD_CDC_B_ACM_SendBuf + usbd_cdc_b_acm_sendbuf_sz - ptr_data_sent;
        }

        if (len_to_send > usbd_cdc_b_acm_maxpacketsize1[USBD_HighSpeed]) {
            /* If
                                                   there is more data to be sent then
                                                   can be sent in a single packet     */
            /* Correct to send maximum pckt size  */
            len_to_send = usbd_cdc_b_acm_maxpacketsize1[USBD_HighSpeed];
        }
    ...
    data_send_zlp = 0;
    /* Send data                          */
    len_sent = USBD_WriteEP(usbd_cdc_b_acm_ep_bulkin | 0x80, ptr_data_sent, len_to_send);
    ptr_data_sent    += len_sent;         /* Correct position of sent pointer   */
    data_to_send_rd  += len_sent;         /* Correct num of bytes left to send  */

    if (ptr_data_sent == USBD_CDC_B_ACM_SendBuf + usbd_cdc_b_acm_sendbuf_sz)
    ...
    if ((data_to_send_wr == data_to_send_rd) &&   /* If there are no more
            ...
            (len_sent == usbd_cdc_b_acm_maxpacketsize1[USBD_HighSpeed])) {
...
/** \brief  Handle Bulk Out Endpoint Events
    ...
    USBD_CDC_B_ACM_EP_BULKOUT_HandleData function to handle received data
...
void USBD_CDC_B_ACM_EP_BULKOUT_Event(uint32_t event)
{
    if (data_read_access) {
...
/** \brief  Handle Bulk In Endpoint Events
    ...
    USBD_CDC_B_ACM_EP_BULKIN_HandleData function to handle send data
    unless data was being accessed in which case function just returns.

    \param [in]         event    Type of event (USBD_EVT_IN - input event).
 */

void USBD_CDC_B_ACM_EP_BULKIN_Event(uint32_t event)
{
...
void USBD_CDC_B_ACM_EP_BULK_Event(uint32_t event)
{
    if (event & USBD_EVT_OUT) {
        USBD_CDC_B_ACM_EP_BULKOUT_Event(event);
    }

    if (event & USBD_EVT_IN) {
        USBD_CDC_B_ACM_EP_BULKIN_Event(event);
    }
}
...
/** \brief  Task Handling Interrupt In Endpoint Events
    ...
    function (USBD_CDC_B_ACM_EP_INTIN_Event).
 */

void USBD_RTX_CDC_B_ACM_EP_INTIN_Event(void)
{
    for (;;) {
        usbd_os_evt_wait_or(0xFFFF, 0xFFFF);
        USBD_CDC_B_ACM_EP_INTIN_Event(usbd_os_evt_get());
    }
}
...
/** \brief  Task Handling Bulk In Endpoint Events
    ...
    function (USBD_CDC_B_ACM_EP_BULKIN_Event).
 */

void USBD_RTX_CDC_B_ACM_EP_BULKIN_Event(void)
{
    for (;;) {
        usbd_os_evt_wait_or(0xFFFF, 0xFFFF);

        if (usbd_os_evt_get() & USBD_EVT_IN) {
            USBD_CDC_B_ACM_EP_BULKIN_Event(0);
        }
    }
}
...
/** \brief  Task Handling Bulk Out Endpoint Events
    ...
    function (USBD_CDC_B_ACM_EP_BULKOUT_Event).
 */

void USBD_RTX_CDC_B_ACM_EP_BULKOUT_Event(void)
{
    for (;;) {
        usbd_os_evt_wait_or(0xFFFF, 0xFFFF);

        if (usbd_os_evt_get() & USBD_EVT_OUT) {
            USBD_CDC_B_ACM_EP_BULKOUT_Event(0);
        }
    }
}
...
/** \brief  Task Handling Bulk In/Out Endpoint Events
    ...
    function (USBD_CDC_B_ACM_EP_BULK_Event).
 */

void USBD_RTX_CDC_B_ACM_EP_BULK_Event(void)
{
    for (;;) {
        usbd_os_evt_wait_or(0xFFFF, 0xFFFF);
        USBD_CDC_B_ACM_EP_BULK_Event(usbd_os_evt_get());
    }
}
#endif
